...
SUPPORTED_PLATFORMS = {
    ...
    "cowork_skill": {
        "name": "Microsoft 365 Copilot Cowork Skill",
        "description": (
            "SKILL.md package deployable to OneDrive at "
            "/Documents/Cowork/skills/{slug}/SKILL.md for Cowork auto-discovery. "
            "Pattern inspired by D365JB/Cowork-DataLoader."
        ),
        "output_files": ["SKILL.md", "skill.json", "deploy.ps1"],
        "best_for": [
            "Individual SE demos",
            "Personal productivity workflows",
            "Zero-infrastructure deployment (just OneDrive)"
        ]
    },
    "mcp_app": {
        "name": "MCP App for M365 Copilot Chat",
        "description": (
            "MCP server with sandboxed HTML widgets rendered inline in Copilot chat "
            "(announced Apr 2026 — modelcontextprotocol.io/extensions/apps/overview)."
        ),
        "output_files": ["server.py", "widgets/*.html", "m365agents.yml", "requirements.txt"],
        "best_for": [
            "M365 Copilot chat",
            "Rich inline UI (tables, forms, KPI tiles)",
            "Bypass Power Automate flow brittleness"
        ]
    }
}
...
class AgentTranspilerAgent(BasicAgent):
    ...
    def _transpile(self, **kwargs) -> str:
        ...
        for platform in platforms_to_generate:
            ...
            elif platform == "cowork_skill":
                results[platform] = self._transpile_to_cowork_skill(agent_def, function_app_url)
            elif platform == "mcp_app":
                results[platform] = self._transpile_to_mcp_app(agent_def, function_app_url)
...
'''
    ...
    # =========================================================================
    # COWORK SKILL TRANSPILER
    # Generates a SKILL.md package deployable to OneDrive at
    # /Documents/Cowork/skills/{slug}/SKILL.md for M365 Copilot Cowork
    # auto-discovery. Pattern adapted from D365JB/Cowork-DataLoader.
    # =========================================================================

    def _transpile_to_cowork_skill(self, agent_def: Dict, function_app_url: str) -> Dict:
        """Transpile a RAPP agent into a Cowork SKILL.md package."""
        agent_info = agent_def.get("agent", agent_def)
        agent_name = agent_info.get("name", agent_info.get("agent_name", "RAPPAgent"))
        slug = self._to_snake_case(agent_name).replace("_", "-")
        title = self._to_title_case(agent_name)
        description = agent_info.get("description", f"RAPP agent: {agent_name}")
        actions = agent_def.get("actions", []) or []
        sample_prompts = (
            agent_info.get("sample_prompts")
            or agent_def.get("sample_prompts")
            or [f"Run {title}", f"Help me with {title.lower()}"]
        )

        # SKILL.md frontmatter + body
        # Cowork reads the YAML frontmatter (name, description) for discovery,
        # then uses the markdown body as the system prompt for the skill.
        steps_md = ""
        if actions:
            steps_md = "\n## Capabilities\n\nThis skill exposes the following actions:\n\n"
            for a in actions:
                a_name = a.get("name") or a.get("action") or "action"
                a_desc = a.get("description", "")
                steps_md += f"- **{a_name}** — {a_desc}\n"

        prompts_md = "\n## Sample Prompts\n\n" + "\n".join(f"- {p}" for p in sample_prompts) + "\n"

        backend_md = (
            "\n## Backend\n\n"
            f"This skill calls the RAPP Function App at `{function_app_url}` for execution. "
            "All actions are routed through the standard RAPP `businessinsightbot_function` "
            "endpoint with `agent_name='" + agent_name + "'`.\n"
        )
        ...
        skill_md = (
            ...
            f"{prompts_md}"
            f"{backend_md}"
            "\n## Output\n\nReturn results as a concise, well-formatted summary. "
            "When listing items, prefer markdown tables. When showing status, use clear "
            "section headers.\n"
        )

        # Manifest entry compatible with Cowork-DataLoader's data/skills.json shape
        skill_manifest = [{
            "owner": "default",
            "skillName": slug,
            "remotePath": f"Documents/Cowork/skills/{slug}/SKILL.md",
            "localFile": f"{slug}/SKILL.md",
            "source": "rapp_transpiler",
            "rappAgent": agent_name
        }]

        # Standalone deploy script — drop SKILL.md into the user's OneDrive.
        # Uses delegated Files.ReadWrite.All. Pattern lifted from
        # D365JB/Cowork-DataLoader/modules/Deploy-CoworkSkills.ps1.
        deploy_ps1 = f"""<#
.SYNOPSIS
    Deploy the {title} Cowork skill to a user's OneDrive.
.DESCRIPTION
    Uploads SKILL.md to /Documents/Cowork/skills/{slug}/SKILL.md.
    M365 Copilot Cowork auto-discovers skills in this folder at the
    start of each conversation.

    Requires: Microsoft.Graph.Authentication module, delegated
    Files.ReadWrite.All consent.

    Pattern adapted from https://github.com/D365JB/Cowork-DataLoader
.PARAMETER UserEmail
    UPN of the user whose OneDrive will host the skill.
#>
param(
    [Parameter(Mandatory)][string]$UserEmail
)

Connect-MgGraph -Scopes 'Files.ReadWrite.All' -NoWelcome | Out-Null

$skillPath  = Join-Path $PSScriptRoot 'SKILL.md'
$remotePath = 'Documents/Cowork/skills/{slug}/SKILL.md'
$content    = Get-Content $skillPath -Raw
$bodyBytes  = [System.Text.Encoding]::UTF8.GetBytes($content)
$encoded    = $remotePath -replace ' ', '%20'
$uri        = "https://graph.microsoft.com/v1.0/users/$UserEmail/drive/root:/$encoded`:/content"

Invoke-MgGraphRequest -Method PUT -Uri $uri -ContentType 'text/plain' -Body $bodyBytes | Out-Null
Write-Host "[OK] Deployed {slug} skill to $UserEmail's Cowork." -ForegroundColor Green
"""

        return {
            "SKILL.md": skill_md,
            "skill.json": skill_manifest,
            "deploy.ps1": deploy_ps1,
        }

    # =========================================================================
    # MCP APP TRANSPILER
    # Generates an MCP server with sandboxed HTML widgets for inline rendering
    # in M365 Copilot chat. See templates/mcp_app/ for the hand-authored
    # baseline this transpiler emits programmatically.
    # =========================================================================

    def _transpile_to_mcp_app(self, agent_def: Dict, function_app_url: str) -> Dict:
        """Transpile a RAPP agent into an MCP App scaffold."""
        agent_info = agent_def.get("agent", agent_def)
        agent_name = agent_info.get("name", agent_info.get("agent_name", "RAPPAgent"))
        snake_name = self._to_snake_case(agent_name)
        description = agent_info.get("description", f"RAPP agent: {agent_name}")
        ...
        # Generate one @mcp.tool() per action, each calling back into the
        # Function App. Widget choice defaults to "table" but can be overridden
        # in the action def via {"widget": "kpi" | "form" | "table"}.
        tool_methods = []
        for a in actions:
            a_name = self._to_snake_case(a.get("name") or a.get("action") or "run")
            a_desc = a.get("description", "")
            widget = a.get("widget", "table")
            tool_methods.append(f'''
@mcp.tool()
async def {a_name}(query: str = "") -> dict:
    """{a_desc}"""
    payload = {{"user_input": query, "agent_name": "{agent_name}", "action": "{a_name}"}}
    async with httpx.AsyncClient() as client:
        r = await client.post(FUNCTION_URL, json=payload, timeout=60)
        r.raise_for_status()
        data = r.json()
    text = data.get("assistant_response", "")
    return {{
        "content": [TextContent(type="text", text=text).model_dump()],
        "_meta": _widget("{widget}.html", {{"title": "{a_name}", "result": data}}),
...
async def run(query: str = "") -> dict:
    """Invoke the RAPP agent with a freeform query."""
    payload = {"user_input": query, "agent_name": "''' + agent_name + '''"}
    async with httpx.AsyncClient() as client:
        r = await client.post(FUNCTION_URL, json=payload, timeout=60)
        r.raise_for_status()
        data = r.json()
    ...
    return {
        "content": [TextContent(type="text", text=text).model_dump()],
        "_meta": _widget("table.html", {"title": "Result", "result": data}),
        "structuredContent": data,
    }
''')

        server_py = f'''"""
MCP App server for RAPP agent: {agent_name}
Auto-generated by AgentTranspiler.

Renders rich HTML widgets inline in M365 Copilot chat via the MCP Apps
spec (modelcontextprotocol.io/extensions/apps/overview, Apr 2026).

Run locally:
    pip install -r requirements.txt
    python server.py

Then register in M365 Agents Toolkit (VS Code):
...
FUNCTION_URL = "{function_app_url}/api/businessinsightbot_function"
WIDGETS_DIR = Path(__file__).parent / "widgets"

mcp = FastMCP(
    name="rapp-{snake_name}",
    instructions="""{description}""",
)


def _widget(template: str, data: dict) -> dict:
    return {{
...
'''
        ...
        # Reuse the widget HTMLs from templates/mcp_app/widgets — but emit
        # them inline so the transpiled output is self-contained.
        table_widget = '''<!doctype html>
<meta charset="utf-8" />
<style>
  body { font: 13px/1.4 'Segoe UI', system-ui, sans-serif; margin: 0; color: #242424; }
...
<div id="root"></div>
<script>
  const data = window.openai?.widgetData || { title: "Result", result: {} };
  const root = document.getElementById("root");
  const text = data.result?.assistant_response || JSON.stringify(data.result, null, 2);
  root.innerHTML = `<h3>${data.title}</h3><pre style="white-space:pre-wrap">${text}</pre>`;
</script>
'''

        kpi_widget = '''<!doctype html>
<meta charset="utf-8" />
<style>
  body { font: 13px/1.4 'Segoe UI', system-ui, sans-serif; margin: 0; }
  .grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 10px; }
  .tile { background: #f5f5f5; border-radius: 6px; padding: 12px; }
  .label { font-size: 11px; color: #616161; text-transform: uppercase; }
  .value { font-size: 22px; font-weight: 600; color: #0078d4; margin-top: 4px; }
</style>
<div id="root"></div>
<script>
  const data = window.openai?.widgetData || { result: {} };
  const kpis = data.result?.kpis || [{label: "Result", value: data.result?.assistant_response || "—"}];
  document.getElementById("root").innerHTML =
    `<div class="grid">` + kpis.map(k =>
      `<div class="tile"><div class="label">${k.label}</div><div class="value">${k.value}</div></div>`
    ).join("") + `</div>`;
</script>
'''

        m365agents_yml = f"""# Microsoft 365 Agents Toolkit manifest
# Generated by RAPP AgentTranspiler for: {agent_name}
version: 1.0.0
name: {snake_name}-mcp
description: {description}
mcp:
...
"""
        ...
        return {
            ...
            "m365agents.yml": m365agents_yml,
            "widgets/table.html": table_widget,
            "widgets/kpi.html": kpi_widget,
            "README.md": (
                f"# {agent_name} — MCP App\n\n"
                f"Auto-generated by RAPP AgentTranspiler.\n\n"
                f"See `templates/mcp_app/README.md` in the RAPP repo for the "
                f"full integration guide. Tracking issue: kody-w/CommunityRAPP#58\n"
            ),
        }
