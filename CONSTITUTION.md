...
When transpiling Python agents (`.py`) to Copilot Studio native solutions, **all metadata MUST be sourced directly from the original `.py` agent files** — never invented, abbreviated, or left empty.

### Required Mappings (Python → Copilot Studio)

| Python Agent Field | Copilot Studio Target | Notes |
|---|---|---|
| `self.metadata["description"]` | `bot.xml` `<description>` tag AND `botcomponent.xml` `<description>` tag | Required for orchestrator routing |
| `self.metadata["description"]` | GPT component `displayName` | Short name shown in UI |
| System prompt / docstring / `self.metadata` | GPT component `instructions` field in `data` file | Full detailed instructions |
...
**The one-liner IS the onboarding.** Every tier of the RAPP platform is installed with a single command. Manual steps exist only as advanced documentation, never as the primary path.

### Rules

1. The brainstem stays clean — CommunityRAPP code never injected at install time
2. CommunityRAPP is for the customer's cloud instance (T2/T3), not the brainstem (T1)
3. The hatchery agent is the on-demand bridge — fetched only when the user asks
4. Projects are isolated — each in `~/rapp-projects/{name}/` with its own venv and port
5. The hatchery agent must be self-contained (stdlib + `requests` only)
6. Progressive disclosure — show only the next step, never the full roadmap

---

## Article XIV — Amendments

This constitution can be amended by opening a PR that modifies `CONSTITUTION.md` with maintainer approval.
